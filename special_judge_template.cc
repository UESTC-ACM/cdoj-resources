...
void dump(bool statement, const int& exit_code) {
  if (statement) return;
  if (node_pointer->data_input != NULL) fclose(node_pointer->data_input);
  if (node_pointer->data_output != NULL) fclose(node_pointer->data_output);
  if (node_pointer->user_output != NULL) fclose(node_pointer->user_output);
  if (exit_code == WA) puts("WA");
  else if (exit_code == AC) puts("AC");
  else if (exit_code == PE) puts("PE");
  else puts("SE");
  delete node_pointer;
  exit(0);
}
